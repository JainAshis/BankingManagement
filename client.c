...
int main(){
	...
	server.sin_addr.s_addr=inet_addr("127.0.0.1");  //loopback address for same machine
	server.sin_port=htons(49854);   // the htons() function converts values between host and network byte orders. These routines convert 16 and 32 bit quantities between network byte order and host byte order. (Network byte order is big endian, or most significant byte first.)
