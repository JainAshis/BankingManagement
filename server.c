...
bool addNormalUser(normalUser record){
    ...
    printf("User ID for added account : %d\n",record.userID);
...
bool deleteNormalUser(int ID){
	...
	if(!strcmp(currUser.status,"ACTIVE")){
		strcpy(currUser.status,"CLOSED");    //change the status to closed
		currUser.balance=0;                   //also change the balance to 0.
...
bool deleteJointUser(int ID){
	...
	if(!strcmp(currUser.status,"ACTIVE")){
		strcpy(currUser.status,"CLOSED");        //change status to closed
		currUser.balance=0;                      //also reduce balance to 0.
...
bool modifyNormalUser(normalUser modUser){
	...
	normalUser currUser;
	lseek(fd,(i)*sizeof(normalUser),SEEK_SET);  //changing the file pointer to the selected record
	read(fd,&currUser,sizeof(normalUser));      //read the user record info from the file.

	if(!strcmp(currUser.status,"ACTIVE") && (modUser.account_no==currUser.account_no)){
		strcpy(modUser.status,"ACTIVE");
		lseek(fd,(-1)*sizeof(normalUser),SEEK_CUR);
		int j=write(fd,&modUser,sizeof(normalUser));     //update the record info as entered by the admin.
		if(j!=0)	result=true;
		else		result=false;
	}
...
void serverTask(int nsd){
	...
	while(1){
		...
		printf("Option : %d\n",option);       //selection of user type during login
	...
	while(1){
		...
		if(option==1 || option==2){                    //actions for normal user and joint users
			if(select==1){
				float amt;
		...
		else if(option==3){                                   //actions for Admin account
...
int main(){
	...
	listen(sd,5);	                            // listen function converts an unconnected socket into a passive socket
	                                          //5 clients per connection// listen returns 0 on success and -1 on error.                                                    
